...
<div class="box">
        <div class="box-header">
           	<h3 class="box-title">{{ $item->brand }} </h3>
        </div>
        <div class="box-body">
      		@if(count($errors) > 0)
			...
			<table class="table table-bordered">
                ...
                <tr>
                  ...
                  <td>{{ $boot->quantity }}</td>
                    ...
                    <input type="hidden" name="stock_id" value="{{$boot->id}}">
                    <input type="hidden" name="stock_name" value="{{$id}}">
                    <input type="hidden" name="stock_details" value="{{ $item->brand }} {{$boot->color}} {{ $boot->bootHeight->height}} size {{ $boot->shoeSize->size }}">

...
<a href="/blouses" class="btn bg-purple">GO BACK</a>
  <h1>Stock Tracking - Blouses</h1>
  <p>Below is a list all stock changes for Blouses.</p>
          ...
          <div class="box-body">
              <table class="table table-bordered">
                  ...
                  <tr>
                      ...
                      <td> {{ $track->variant }} </td>
                      @if($track->blouse->company == 'Plain')
                      <td>{{ $track->blouse->color }} {{ $track->blouse->company }}</td>
                      @else
                      <td>{{ $track->blouse->color }} Printed {{ $track->blouse->company }}</td>
                      @endif
                      <td> {{ $track->blouse->size }} </td>
